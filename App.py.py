...
def intrusion_prediction(input_data):
    ...
    for i in range(min(10, len(input_data))):
        ...
        if prediction[0] == 0:
            predictions.append(('normal', 'The network activity in row {} is normal'.format(i+1)))
        else:
            predictions.append(('intrusion', 'The network activity in row {} is an intrusion'.format(i+1)))
    return predictions
